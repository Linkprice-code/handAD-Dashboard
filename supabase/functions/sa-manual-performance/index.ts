...
//
// sa-manual-upload가 수기 업로드로 채운 sa_manual_campaign_raw / sa_manual_keyword_raw /
// sa_manual_product_raw에서, 세션 토큰으로 검증된 advertiser_id의 행만 날짜 범위(있으면)
// 안에서 가져와 집계해 돌려준다. gfa-performance / (기존) sa-performance와 동일한 응답
// 형태를 최대한 유지해서 프론트엔드가 API 자동 동기화용 응답과 동일하게 다룰 수 있게 한다.
